...
SLOT_BYTES = 32  # each WOTS pk value is a 256-bit int, top 128 bits meaningful (N_MASK)

# def _worker_init(shm_name, seed, sk_seed, layer, tree, cfg, hash_backend):
#     global _SHM,_BUF, _SEED, _SK_SEED, _LAYER, _TREE, _CFG, HASH_BACKEND
#     HASH_BACKEND = hash_backend
#     _SHM = shared_memory.SharedMemory(name=shm_name)
#     _BUF = _SHM.buf
#     _SEED, _SK_SEED, _LAYER, _TREE, _CFG = seed, sk_seed, layer, tree, cfg


# ============================================================
# Shared worker init — handles BOTH pk-only and full-keygen modes.
# sk_shm_name=None -> pk-only mode (build_subtree_root_parallel)
# sk_shm_name set  -> full mode (build_subtree_full_parallel)
# ============================================================
def _worker_init(pk_shm_name, sk_shm_name, seed, sk_seed, layer, tree, cfg, hash_backend, l):
    global _PK_SHM, _SK_SHM,_PK_BUF, _SK_BUF, _SEED, _SK_SEED, _LAYER, _TREE, _CFG, _L, HASH_BACKEND
    HASH_BACKEND = hash_backend

    # pk_shm = shared_memory.SharedMemory(name=pk_shm_name)
    _PK_SHM = shared_memory.SharedMemory(name=pk_shm_name)   # ← keep the object itself alive globally
    _PK_BUF = _PK_SHM.buf

    if sk_shm_name is not None:
        # sk_shm = shared_memory.SharedMemory(name=sk_shm_name)
        _SK_SHM = shared_memory.SharedMemory(name=sk_shm_name)  # ← same here
        _SK_BUF = _SK_SHM.buf
    else:
        _SK_SHM = None
        _SK_BUF = None

    _SEED, _SK_SEED, _LAYER, _TREE, _CFG, _L = seed, sk_seed, layer, tree, cfg, l



def _worker_compute(kp):
    pk = wots_keygen_pk_only(_SEED, _SK_SEED, _LAYER, _TREE, kp, _CFG)
    # Each kp writes to its OWN disjoint slice — safe, no lock required
    _PK_BUF[kp * SLOT_BYTES:(kp + 1) * SLOT_BYTES] = pk.to_bytes(SLOT_BYTES, "big")

def _worker_compute_full(kp):
    """Full-keygen worker — writes pk into _PK_BUF and all l sk elements into _SK_BUF."""
    sk, pk = wots_keygen(_SEED, _SK_SEED, _LAYER, _TREE, kp, _CFG)

    _PK_BUF[kp * SLOT_BYTES:(kp + 1) * SLOT_BYTES] = pk.to_bytes(SLOT_BYTES, "big")

    base = kp * _L * SLOT_BYTES  # each kp gets its own disjoint L-slot block, no overlap with other kp's
    for i, sk_i in enumerate(sk):
        off = base + i * SLOT_BYTES
        _SK_BUF[off:off + SLOT_BYTES] = sk_i.to_bytes(SLOT_BYTES, "big")


def build_subtree_root_parallel(seed, sk_seed, layer, tree, subtree_h, cfg=None, n_workers=None):
    n_leaves = 1 << subtree_h
    n_workers = n_workers or os.cpu_count()
    # n_workers = 16

    pk_shm = shared_memory.SharedMemory(create=True, size=n_leaves * SLOT_BYTES)
    try:
        chunksize = max(1, n_leaves // (n_workers * 8))
        with mp.Pool(
            processes=n_workers,
            initializer=_worker_init,
            initargs=(pk_shm.name,None, seed, sk_seed, layer, tree, cfg, HASH_BACKEND,0),
        ) as pool:
            # imap_unordered: workers finish and write whenever ready, order doesn't matter
            # because each one is writing to a distinct, pre-committed slot.
            for _ in pool.imap_unordered(_worker_compute, range(n_leaves), chunksize=chunksize):
                pass

        leaves = [
            int.from_bytes(bytes(pk_shm.buf[i * SLOT_BYTES:(i + 1) * SLOT_BYTES]), "big")
...
# ============================================================
# Full-keygen parallel build (new — returns wots_sks too)
# ============================================================
def build_subtree_full_parallel(seed, sk_seed, layer, tree, subtree_h, cfg=None, n_workers=None):
    """Parallel version of build_subtree_full. Returns (wots_sks, tree_nodes, root)."""
    n_leaves = 1 << subtree_h
    n_workers = n_workers or os.cpu_count()
    # n_workers = 16

    _, _, l, _, _, _ = _wots_params(cfg)  # chain count (32 or 43 depending on variant)

    pk_shm = shared_memory.SharedMemory(create=True, size=n_leaves * SLOT_BYTES)
    sk_shm = shared_memory.SharedMemory(create=True, size=n_leaves * l * SLOT_BYTES)
    try:
        chunksize = max(1, n_leaves // (n_workers * 8))
        with mp.Pool(
            processes=n_workers,
            initializer=_worker_init,
            initargs=(pk_shm.name, sk_shm.name, seed, sk_seed, layer, tree, cfg, HASH_BACKEND, l),
        ) as pool:
            for _ in pool.imap_unordered(_worker_compute_full, range(n_leaves), chunksize=chunksize):
                pass
        ...
        wots_sks = [
            [
                int.from_bytes(
                    bytes(sk_shm.buf[(kp * l + i) * SLOT_BYTES:(kp * l + i + 1) * SLOT_BYTES]), "big"
                )
                for i in range(l)
            ]
            for kp in range(n_leaves)
...
def _fors_tree_worker(args):
    """Builds ONE full FORS tree (leaves + all Merkle levels) — the entire
    per-tree workload, in one process. Returns (t, tree_nodes, root)."""
    t, seed, sk_seed, a, cfg, ht_idx, idx_leaf0, idx_tree0, mode, hash_backend = args
    global HASH_BACKEND
    HASH_BACKEND = hash_backend  # each process needs this set independently
    tree_nodes, root = build_fors_tree(seed, sk_seed, t, a, cfg,
                                        ht_idx, idx_leaf0, idx_tree0, mode)
    return t, tree_nodes, root
    

def fors_sign_full_parallel(seed, sk_seed, digest, k, a, cfg=None, n_workers=None):
    """Parallel version of fors_sign_full — one worker builds one full FORS
    tree end-to-end (leaves + Merkle levels). Result-swap-in is done via
    pre-declared, task-index-mapped slots, so each worker's output always
    lands in its own reserved position, never overlapping another's."""
    mk_adrs, _, _ = _adrs_helpers(cfg)
    n_workers = n_workers or min(k, os.cpu_count())
    ...
    # Pre-declared result slots, one per tree index t. Each worker writes
    # ONLY to its own t — no two tasks ever target the same slot.
    tree_nodes_by_t = [None] * k
    root_by_t = [None] * k

    tasks = [
        (t, seed, sk_seed, a, cfg, ht_idx, idx_leaf0, idx_tree0, mode, HASH_BACKEND)
        for t in range(k)  # k independent trees total (k-1 "real" + 1 forced-zero)
    ]

    with mp.Pool(processes=n_workers) as pool:
        for t, tree_nodes, root in pool.imap_unordered(_fors_tree_worker, tasks):
            tree_nodes_by_t[t] = tree_nodes
            root_by_t[t] = root

    # ---- Everything below is cheap, sequential post-processing ----
    # (secret extraction, auth-path lookup, and the final th_multi — all
    # tiny compared to tree construction, not worth parallelizing.)
    secrets, auth_paths, roots = [], [], []
    for t in range(k - 1):
        eprint(f"  Parrallelized FORS tree {t}/{k-1}...")
        secrets.append(fors_secret(sk_seed, t, indices[t], ht_idx))
        auth_paths.append(get_auth_path(tree_nodes_by_t[t], indices[t], a))
        roots.append(root_by_t[t])

    root_last = root_by_t[k - 1]
    secrets.append(root_last)
...
def sign_variant(variant_name, message_int, seed=None, sk_seed=None, pk_root=None):
    ...
    if scheme == "fors":
        ...
        fors_secrets, fors_auth_paths, bottom_pk = fors_sign_full_parallel(seed, sk_seed, digest, k, a, cfg)
    else:
    ...
    for lay in range(d):
        ...
        wots_sks, tree_nodes, _ = build_subtree_full_parallel(seed, sk_seed, lay, idx_tree, subtree_h, cfg)
...
def _build_hypertree_d2(seed, sk_seed, subtree_h, cfg=None):
    ...
    pk_root = build_subtree_root_parallel(seed, sk_seed, 1, 0, subtree_h, cfg)

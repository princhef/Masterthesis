...
def wots_keygen_pk_only(seed, sk_seed, layer, tree, kp, cfg=None):
    ...
    for i in range(l):
        ...
        eprint(f"  secret calc  : {time.time()-t0:.10f}s")

        chain_adrs = set_ci(base_adrs, i)
        t0=time.time()
        pk_i = ch_hash(seed, chain_adrs, sk_i, 0, w - 1)
        eprint(f"  chain pk hash calc  : {time.time()-t0:.10f}s")
...
def build_subtree_root(seed, sk_seed, layer, tree, subtree_h, cfg=None):
    ...
    for kp in range(n_leaves):
        ...
        leaves.append(pk)
        print(kp)
        eprint(f"  Leaves calc  : {time.time()-t0:.10f}s")

    eprint(f"  Leaves calculated in : {time.time()-t1:.1f}s")
...
def sign_variant(variant_name, message_int, seed=None, sk_seed=None, pk_root=None):
    ...
    eprint(f"  FORS grinding: {time.time()-t0:.1f}s")
    ...
    eprint(f"  FORS signing: {time.time()-t0:.1f}s")
    ...
    eprint(f"  Hypertree Signing: {time.time()-t0:.1f}s")
    # ================================================================
